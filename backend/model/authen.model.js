...
export default {
  ...
  getUidFromReqWs(reqWs) {
    if (reqWs.headers.cookie) {
      const cookies = parse(reqWs.headers.cookie);
      return this.getUidFromToken(cookies[CookieModel.ACCESS_TOKEN]);
    }
    return null;
  },
  ...
  genConfirmationToken(uid, ttl = verifyExpireTime) {
    const buff = Buffer.from(genTokenCustomTime({ uid }, ttl));
  ...
  genTAInvitationToken(email, ttl = verifyExpireTime) {
    const buff = Buffer.from(genTokenCustomTime({ email }, ttl));
    return buff.toString("base64");
  },

  verifyTAInvitationToken(tokenBase64) {
